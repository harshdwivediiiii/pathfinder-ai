...
export async function runAiGeneration(prompt, options = {}) {
  ...
  try {
    const cached = await getCachedResponse(userId, prompt);
    if (cached) {
      incrementCacheHit();
    ...
    await cacheResponse(userId, prompt, text);
    span.setAttribute("cache.hit", false);
    span.end();
    ...
    const degradedCache = await getCachedResponse(userId, prompt);
    if (degradedCache) {
      logger.warn("Serving degraded cached response after failure", { userId });
      setCircuitBreakerState("degraded");
      return { degraded: true, text: degradedCache, source: "degraded-cache" };
    }
...
export async function runAiGenerationStream(prompt, options = {}) {
  ...
  try {
    ...
    const finalize = async () => {
      if (fullText.trim()) {
        await cacheResponse(userId, prompt, fullText);
      }

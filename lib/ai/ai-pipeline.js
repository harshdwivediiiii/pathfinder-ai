...
export async function runAiGeneration(prompt, options = {}) {
  ...
  span.setAttribute("userId", userId ?? "anonymous");

  try {
    const cached = userId ? await getCachedResponse(userId, prompt, options.generationConfig) : null;
    if (cached) {
      incrementCacheHit();
    ...
    const degradedCache = userId ? await getCachedResponse(userId, prompt, options.generationConfig) : null;
    if (degradedCache) {

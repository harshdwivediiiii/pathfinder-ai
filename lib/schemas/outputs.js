...
export const resumeRoastOutputSchema = z.object({
  roast: z.string().min(10).max(2000),
  score: z.number().min(0).max(100),
  fixes: z.array(z.string().min(5).max(500)).min(1).max(10),
}).strict();

export const onboardingPlanOutputSchema = z.object({
  day30: z.object({
    focus: z.string().min(3).max(200),
    goals: z.array(z.string().min(5).max(300)).min(1).max(10),
  }).strict(),
  day60: z.object({
    focus: z.string().min(3).max(200),
    goals: z.array(z.string().min(5).max(300)).min(1).max(10),
...
export const SCHEMA_DESCRIPTIONS = {
  ...
  resumeRoast: `{
  "roast": "string (2-3 paragraph brutal roast)",
  "score": 42,
  "fixes": ["string (actionable fix)", "string (actionable fix)", "string (actionable fix)"]
}`,
  onboardingPlan: `{
  "day30": { "focus": "string", "goals": ["string", "string"] },
  "day60": { "focus": "string", "goals": ["string", "string"] },
  "day90": { "focus": "string", "goals": ["string", "string"] }
}`,
  imposterSyndrome: `{

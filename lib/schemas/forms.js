...
export const salaryNegotiationChatSchema = z.object({
  history: z.array(
    z.object({
      role: z.enum(["user", "model"], {
  ...
  userMessage: z.string().min(1, "Message cannot be empty.").max(2000, "Message is too long."),
}).strict();

export const salaryNegotiationEvaluateSchema = z.object({
  history: z.array(
    z.object({
      role: z.enum(["user", "model"], {
        errorMap: () => ({ message: "Invalid role." }),

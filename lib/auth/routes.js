...
export const isPublicRoute = createRouteMatcher([
  ...
  "/api/inngest",
  // Liveness/readiness probes (docker-compose.yml, DEPLOYMENT.md) must work
  // without a session. The metrics sub-route stays protected.
  "/api/health",
  "/api/health/ready",
  "/explore(.*)",
  "/compare(.*)",
  ...
  export async function getAuthDecision(req, auth) {
    ...
    if (/^\/interview\/video-coach(?:$|\/)/.test(req.nextUrl.pathname)) {
      ...
      // Not allowed (e.g. production) -> fall through to normal auth checks.
    }

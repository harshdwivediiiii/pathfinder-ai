...
export function getOrCreateInFlightInsight(industry, generator) {
  const existing = inFlightInsights.get(industry);
  if (existing) {
    return existing;
  }

  const promise = generator().finally(() => {
    if (inFlightInsights.get(industry) === promise) {
      inFlightInsights.delete(industry);
    }
  });

  inFlightInsights.set(industry, promise);
  return promise;
}

  ...
  for (const job of dueJobs) {
    ...
    try {
      await step.run(`Send reminder ${job.id}`, async () => {
        // Persist the reminder flag before sending so the idempotency guard
        // (interviewReminderSentAt: null in the query) protects against
        // re-sends even if a retry runs between the two awaits.

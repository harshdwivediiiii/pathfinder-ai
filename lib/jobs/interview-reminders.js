...
export async function processInterviewRemindersJob({
  ...
  for (const job of dueJobs) {
    ...
    try {
      const claimed = await step.run(`Send reminder ${job.id}`, async () => {
        // Atomically claim this interview before sending. Only the run that
        // transitions interviewReminderSentAt from null to now wins
        // (count === 1); a concurrent run that already claimed it sees
        // count === 0 and skips the email, so retries/overlaps cannot
        // double-deliver.
        const claim = await dbClient.jobApplication.updateMany({
      ...
      logLogger.error("send-interview-reminders-cron", error, { jobId: job.id });
    }
  }

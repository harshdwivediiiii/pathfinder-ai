    ...
    _workerFnPromise = (async () => {
      ...
      return inngest.createFunction(
        ...
        async ({ event, step }) => {
          ...
          const { user, roadmap, industryInsight } = await step.run("Fetch user and roadmap", async () => {
            ...
            const industryInsight = user?.industry
              ...
              : null;
            return { user, roadmap, industryInsight };
          });
          ...
          // Run LangChain agent against curated / insight-backed market data
          ...
          if (!updatedRoadmapData?.adapted) {
            log.info("process-roadmap-adaptation", "Adaptation skipped; preserving roadmap", {
              userId,
              reason: updatedRoadmapData?.reason,
            });
            return {
              userId,
              updated: false,
              preserved: true,
              reason: updatedRoadmapData?.reason || "market_data_unavailable",
            };
          }
          ...
          log.info("process-roadmap-adaptation", "Roadmap adapted successfully", {
            userId,
            source: updatedRoadmapData.source,
            catalogVersion: updatedRoadmapData.catalogVersion,
          });

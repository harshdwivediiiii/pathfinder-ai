import "server-only";
import { createRateLimitStore, refillBucket } from "@/lib/security/rate-limit/store";
...
export async function atomicIncrement(key, store = getDefaultStore()) {
  const mutex = (await import("@/lib/security/rate-limit/mutex")).getGlobalBucketMutex();

...
function getStats(endpoint) {
  if (!stats.has(endpoint)) {
    stats.set(endpoint, { attempts: 0, rejected: 0, lastAccess: Date.now() });
  }
...
function cleanupStaleStats(now = Date.now()) {
  let cleaned = 0;
  for (const [key, entry] of stats.entries()) {
    if (entry.lastAccess && now - entry.lastAccess > STATS_TTL_MS) {
      stats.delete(key);
      cleaned++;
    }
  }
...
export function extractTrustedClientIp(headers) {
  ...
  // Only trust X-Forwarded-For when TRUSTED_PROXY_COUNT is explicitly configured.
  // Defaulting to 1 would allow attackers to forge headers when there is no proxy.
  const parsedCount = Number(TRUSTED_PROXY_COUNT);
  if (!(parsedCount > 0)) {
    return "unknown";
  }
  ...
  const trustedProxyCount = parsedCount;
...
export async function enforceRateLimit({
  ...
  if (typeof store.checkAndDeduct === "function") {
    try {
      ...
      // Store failure policy: fail-closed by default so a rate-limit store
      // outage cannot turn into unlimited AI generation. Fail-open is only
      // applied when explicitly opted in via RATE_LIMIT_FAIL_OPEN=true.
      const failOpen = getEnv().RATE_LIMIT_FAIL_OPEN;
      console.error(
        `[rate-limit] checkAndDeduct failed, ${failOpen ? "allowing" : "rejecting"} request:`,
        err.message
      );
      ...
      return {
        allowed: failOpen,
        remaining: 0,
        retryAfterSeconds: failOpen ? 0 : 60,
        rejectionRate:
          statsEntry.attempts === 0
            ? 0
            : statsEntry.rejected / statsEntry.attempts,
  ...
  try {
    ...
    const failOpen = getEnv().RATE_LIMIT_FAIL_OPEN;
    console.error(
      `[rate-limit] Fallback checkAndDeduct failed, ${failOpen ? "allowing" : "rejecting"} request:`,
      err.message
    );
    statsEntry.rejected += 1;
    return {

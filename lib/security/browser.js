...
export function hasWebStorage() {
  ...
  try {
    ...
    console.warn("[browser] Web storage detection failed:", err);
    return false;
  }
}
...
async function decryptValue(encoded) {
  try {
    ...
    console.warn("[browser] Decryption failed:", err);
    return null;
  }
}
...
export const safeLocalStorage = {
  async getItem(key) {
    ...
    try {
      ...
      console.warn("[browser] localStorage getItem failed:", err);
      return null;
    }
  },
  async setItem(key, value) {
    ...
    try {
      ...
      console.warn("[browser] localStorage setItem failed:", err);
    }
  },
  removeItem(key) {
    if (!hasWebStorage()) return;
    try {
      ...
      console.warn("[browser] localStorage removeItem failed:", err);
    }
  },
};
...
export const safeSessionStorage = {
  getItem(key) {
    ...
    try {
      ...
      if (sessionStorage === null || sessionStorage === undefined || typeof sessionStorage.getItem !== "function") {
        return null;
      }
      return sessionStorage.getItem(key);
    } catch (err) {
      console.warn("[browser] sessionStorage getItem failed:", err);
      return null;
    }
  },
  setItem(key, value) {
    ...
    try {
      ...
      console.warn("[browser] sessionStorage setItem failed:", err);
    }
  },
  removeItem(key) {
    if (!isBrowser()) return;
    try {
      ...
      console.warn("[browser] sessionStorage removeItem failed:", err);
    }
  },
};

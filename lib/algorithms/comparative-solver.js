...
export class ComparativeSolver {
  ...
  compareResults(results, options = {}) {
    ...
    const solved = results.filter((r) => r.success);

    // Normalize the cost term across the candidate result set instead of
    // against each result's own cost. Self-normalization makes costScore always
    // 0 for any path with cost > 1, which silently removes the cost (and
    // speed/risk) weights from the ranking.
    const costs = solved.map((r) => (isFinite(r.result.cost) ? r.result.cost : 999999));
    const maxCost = costs.length > 0 ? Math.max(...costs) : 999999;
    const minCost = costs.length > 0 ? Math.min(...costs) : 0;

    const scored = solved
      .map((r) => {
        const score = this.weightedCompositeScore(r.result, weights, { maxCost, minCost });
        return { algorithm: r.algorithm, score, path: r.result.path, cost: r.result.cost };
      })
      .sort((a, b) => b.score - a.score);

    const paretoFrontier = this.computeParetoFrontier(solved);
  ...
  weightedCompositeScore(result, weights, costRange = {}) {
    const safeCost = isFinite(result.cost) ? result.cost : 999999;
    const maxCost = costRange.maxCost ?? Math.max(safeCost, 1);
    const minCost = costRange.minCost ?? 0;
    const rangeSize = maxCost - minCost;
    // Set-relative cost score: 1 for the cheapest candidate, 0 for the most
    // expensive. When the range is zero (single result or equal costs) every
    // candidate is equally cheap and gets costScore 1.
    const costScore = rangeSize > 0 ? Math.max(0, Math.min(1, (maxCost - safeCost) / rangeSize)) : 1;

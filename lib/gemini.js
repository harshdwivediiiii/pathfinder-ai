...
async function generateGeminiText(prompt, { timeout = 30000 } = {}) {
  const result = await generateGeminiContent(prompt, { timeout });
  return result.response.text().trim();
}

export async function generateGeminiStructuredContent({
  prompt,
  schema,
  schemaName = "response",
  jsonSchemaExample,
  ...
  const firstAttempt = await generateText(prompt, { timeout });
  let validated = validateStructuredOutput(firstAttempt, schema, schemaName);
  ...
  const correctionPrompt = buildFormatCorrectionPrompt({
    schemaName,
    jsonSchemaExample,
    previousResponse: firstAttempt,
    additionalRules: correctionRules,
  });

  const secondAttempt = await generateText(correctionPrompt, { timeout });
  validated = validateStructuredOutput(secondAttempt, schema, schemaName);
  ...
  throw new GeminiError(
    `AI returned invalid ${schemaName} format after one correction attempt`,
    "UNKNOWN"
  );
}
